...
// For Product related API
  ...
  checkValidation() {
    ...
    if(!this.selectedPaymentMode && !this.dual_payment) {
      ...
      this.formError = true;
        ...
        if(this.name) {
           this.findCustomerBookingDetail(this.name,billId)
        } else {
  ...
  async findCustomerBookingDetail(customer_name,billId) {
    let loader = await this.loading.create({
    ...
    loader.present().then(() => {
      ...
      this.appointmentService.getAppointmentOfBillingCustomer(customer_name).subscribe((res) => {
         if(res.data) {
  ...
  async showGenerateBill(id){
     ...
     for(let item of this.salonSelectedItems) {
      ...
      //  let tempServArr = this.servArr.find((item) => item)
      // for(let i=0;i<this.servArr.length;i++) {
      //   for(let j=0;j<this.servArr.length;j++) {


      //       if(this.servArr[i].service_id === this.servArr[j].service_id && this.servArr[i].total === this.servArr[j].total) {
